...
const TopCompany: React.FC = () => {
    ...
    const topCompanies = mockCompanies.filter(company => company.isTopCompany);

    // Tính toán số trang
    const totalPages = Math.ceil(topCompanies.length / companiesPerPage);

    // Lấy các công ty cho trang hiện tại
    const currentTopCompanies = topCompanies.slice(
    ...
    return (
        <Container>
            <HeaderSection>
                ...
                <CompanyCount>Top {topCompanies.length} doanh nghiệp</CompanyCount>

            </HeaderSection>

            <CompanyGrid>
                {currentTopCompanies.map((company) => (
                    <CompanyCard key={company.id}>
                        <div className="flex gap-4">
                            <CompanyLogoSection>
                                <CompanyLogo src={company.logo} alt={company.name} />
                                <TopBadge><img src="/images/home/TOP.png" alt="Top Badge" /></TopBadge>
                            </CompanyLogoSection>
                            <div>
...
const ActionButtons = styled.div`
  ...
  /* --- Tablet dọc (<= 768px) --- */
  @media (max-width: 768px) {
    flex-wrap: wrap; /* Cho phép xuống hàng nếu nút dài */
    justify-content: space-between;
