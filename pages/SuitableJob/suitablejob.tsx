...
const suitableJobs: SuitableJob[] = [
    {
        id: '1',
...
const SuitableJob: React.FC = () => {
    const [suitableJobsList, setSuitableJobsList] = useState(suitableJobs);

    // Xử lý xóa công việc khỏi danh sách đã lưu
    const handleRemoveFromSaved = (jobId: string) => {
        setSuitableJobsList(prev => prev.filter(job => job.id !== jobId));
    };
        ...
        <Container>
            <MainContent>
                ...
                <Header>
                    ...
                    <div className="text-[#007AFF] font-medium">{suitableJobsList.length} doanh nghiệp</div>
                </Header>

                {/* Applied Jobs List */}
                <JobsList>
                    {suitableJobsList.map((job) => (
                        <JobCard key={job.id}>
                            <JobHeader>
                                <CompanyInfo>
                                    <CompanyLogo
                            ...
                            <div className="flex justify-between items-center mt-4">
                                ...
                                <JobActions>
                                    ...
                                    <RemoveButton onClick={() => handleRemoveFromSaved(job.id)}>
                                        <img src="/images/home/Yêu thích.png" alt="Xóa" />
                                    </RemoveButton>
                                </JobActions>
                            </div>
                        </JobCard>
