...
const ChiTiet = () => {
	...
	const [dataChiTiet, setDataChiTiet] = useState<any>({
		contentEditor:
			"Phát biểu tại buổi Lễ, TS. Vũ Tuấn Lâm, Phó Giám đốc Học viện nhấn mạnh, việc tổ chức các cuộc thi trong sinh viên nhằm tạo sân chơi cho sinh viên, là nơi để sinh viên gặp gỡ, giao lưu và trao đổi những kiến thức đã học, khuyến khích phong trào nghiên cứu khoa học trong sinh viên. Phó Giám đốc Học viện cũng khẳng định: “Khoa Kỹ thuật Điện tử 1 là một trong những khoa đào tạo thế mạnh của Học viện, với tỷ lệ sinh viên sau khi ra trường có việc làm chiếm tỷ lệ cao. Việc đẩy mạnh các hoạt động ngoại khóa, các cuộc thi cho sinh viên cũng nhằm nâng cao chất lượng đào tạo và cung cấp thêm các kỹ năng mềm cho sinh viên”.",
	});
	const [dataLienQuan, setDataLienQuan] = useState<TuyenSinh[]>([]);
	...
	// 	}
	// }
	const getDataLienQuan = async () => {
		try {
			const res = await axios.post(`${ip}/cmscore/v5/Article/GetData/GetArticleByRelCategory/${router?.query?.type}`, {
				filters: [],
				sorts: [],
				pageInfo: {
					page: 1,
			...
			<div className='container mx-auto bg-white px-6 pt-6 pb-8' ref={contentRef}>
				...
				<div className={" mt-14 "}>
					<div className={"detail mb-14 flex flex-col items-center border-b border-[#C3C7CC]"} >
						<h2>Lễ khai mạc cuộc thi “ Lập trình Robot năm 2023” </h2>
						<div className='flex justify-center'>
							<p className='date lg:mr-[40px] mr-[20px]'>
								Ngày đăng: {moment(dataChiTiet?.created).format("DD/MM/YYYY HH:mm")}
							</p>
					...
					{/*/>*/}
					<div className='content' dangerouslySetInnerHTML={{ __html: dataChiTiet?.contentEditor ?? "" }}></div>
